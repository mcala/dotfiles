...
Please assume that `uv` is already installed and available to you on `PATH`.

## 0 - Sanity Check

```bash
uv --version    # verify that uv is installed; exits 0
```

If this command fails, **STOP** and report to the user.

## uv --help output

Below is the output to uv --help.
...
Global options:
  ...
  -v, --verbose...
      ...
      --config-file <CONFIG_FILE>
          ...
          UV_CONFIG_FILE=]
      --no-config
          Avoid discovering configuration files (`pyproject.toml`, `uv.toml`) [env:
          UV_NO_CONFIG=]
  -h, --help
