...
# ]
# ///
# ABOUTME: Builds a styled Excel spreadsheet from a video/image list analysis
# ABOUTME: JSON file, with numbered rows, auto-fit columns, and a notes sheet.
"""
Build a styled Excel spreadsheet from a video list analysis JSON file.
...
# ── style constants (matching project convention) ─────────────────
...
def estimate_column_width(header: str, values: list[str]) -> float:
    ...
    return min(50.0, max(15.0, max_len + 4))
...
def build_workbook(analysis: dict[str, object]) -> openpyxl.Workbook:
    ...
    source: str = str(analysis.get("source", ""))

    # Sheet title from list_type, capitalized
    ...
    all_headers = ["#"] + columns
    all_col_widths: list[float] = [6.0]  # width for # column
...
def main() -> None:
    ...
    if args.output:
        ...
        # Derive from source video name if available, else from input JSON stem
        source = analysis.get("source", "")
        if source:
            output_path = input_path.parent.joinpath(Path(source).stem + ".xlsx")
        else:
